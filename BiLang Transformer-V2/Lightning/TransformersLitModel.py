...
class BilangLightning(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # self.log("val/loss", mean_val_loss, prog_bar=True)
    ...
    def on_validation_epoch_end(self) -> None:
        if self.epoch_loader > 0:
            ...
            self.run_validation(self.net, self.last_val_batch)

            # mean_val_loss = sum(self.val_loss) / len(self.val_loss)
            # print("Validation Loss : ", round(mean_val_loss.item(), 5))
    ...
    def run_validation(self, model, data):
        ...
        bleu = self.bleu_score(predicted, expected)
